...
def create_secret(service_client, arn, token):
    ...
    try:
        ...
        req = requests.post(f"{ED_API_URL}/token",
                            json={"login": secret["ed_login"], "password": secret["ed_password"]})

        if req.status_code != 200:
            raise Exception("Ed Authentication Failed.")

        ed_token = req.json()["token"]
        ...
        service_client.put_secret_value(SecretId=arn, ClientRequestToken=token,
                                        SecretString=json.dumps({
                                            "ed_login": secret["ed_login"],
                                            "ed_password": secret["ed_password"],
                                            "ed_token": ed_token,
...
def set_secret(service_client, arn, token):
    ...
    # We are just updating the token, not anything service-side (i.e. in Ed or Discord).
    return
...
def test_secret(service_client, arn, token):
    ...
    req = requests.get(secret["discord_webhook_url"])
    if req.status_code != 200:
        raise Exception(f"Discord Webhook URL ({secret['discord_webhook_url']}) GET did not succeed.")
    
    req = requests.get(f"{ED_API_URL}/user", headers={"x-token": secret["ed_token"]})
    if req.status_code != 200:
        raise Exception("Ed user endpoint get request did not succeed.")
